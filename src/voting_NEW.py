...
def check_human_team_success(gs: GameState):
    # Get all AI players that started in the game (whether voted out or not)
    all_ai_players = [p for p in gs.players + gs.players_voted_off if not p.is_human]

    # Get how many AI players were voted out
    voted_out_ais = [p for p in gs.players_voted_off if not p.is_human]

    print(
        Fore.CYAN +
        f"The human team voted out {len(voted_out_ais)} out of {len(all_ai_players)} AI players!".upper() +
        Style.RESET_ALL
    )
#########################################################################
